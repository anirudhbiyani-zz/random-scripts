...
def main():
    ...
    for user in users:
        ...
        for access_key in access_keys:
            ...
            #    client.update_access_key(UserName=username, AccessKeyId=access_key_id, Status="Inactive")
                ...
                response = sclient.send_email(Source=EMAIL_FROM, Destination={'ToAddresses': [email_to]},
                    Message={'Subject': {'Data': 'Deactivation of Access Key: %s' % userid},
                        'Body': { 'Text': { 'Data': data}}})
    ...
    response = sclient.send_email(Source=EMAIL_FROM, Destination={'ToAddresses': 'infosec@khoros.com'},
    Message={'Subject': {'Data': 'AWS IAM Access Key Rotation - Lambda Function'},
        'Body': { 'Text': { 'Data': data}}})
